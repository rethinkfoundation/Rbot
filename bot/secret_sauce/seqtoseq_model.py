...
def seqtoseq_train(n_iters, training_data,print_every=1000, learning_rate=0.01, tfl=False):
    ...
    metadata = open('bot/brain/seqtoseq_meta.pkl', 'wb')
    ...
    if tfl == False:
       ...
       encoder = torch.load('bot/brain/encoder.pt')
       decoder = torch.load('bot/brain/decoder.pt')
    if use_cuda:
    ...
    for iter in range(1, n_iters + 1):
        ...
        if iter%1000 == 0:
           print(accuracy,"%")
    torch.save(encoder, 'bot/brain/encoder.pt')
    torch.save(decoder, 'bot/brain/decoder.pt')
...
def reply_predict(sentence):
    try:
     encoder = torch.load('bot/brain/encoder.pt')
     decoder = torch.load('bot/brain/decoder.pt')
     with open('bot/brain/seqtoseq_meta.pkl','rb') as pickle_file:
